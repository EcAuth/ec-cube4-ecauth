...
## オーナーズストアの商品ページを更新するとき

オーナーズストアの商品ページ（`composer.json` の `extra.id` = 3557）にある
「詳細説明 / 詳細コメント」欄は**制約が多い**。リリースノートや README をそのまま
コピペすると崩れるので、以下を踏まえて書き直すこと。

### 改行は nl2br で `<br />` になる

入力した改行はすべて `<br />` に変換されて出力される。HTML をインデントしたり
ブロック要素ごとに改行して貼ると、**要素の間に余計な空行が大量に入る**。

→ **改行を 1 つも含まない 1 行の HTML として書く**。改行したい箇所だけ `<br />` を明示する。

### Markdown は展開されない

`#### 見出し` / `**強調**` / バッククォート / `> 引用` は、そのまま文字として画面に出る。
リリースノートを流用するときは必ず HTML へ変換すること。

### 使えるタグは限られる（`<code>` は弾かれる）

保存時にホワイトリスト検証があり、弾かれるとこのエラーになる。

```
※ 詳細説明 / 詳細コメント1に許可されていないタグ [code], [code], ... が含まれています。
```

2026-08 時点で**通ることを確認したタグ**: `div` / `p` / `strong` / `ul` / `li` / `br` / `a`。
`<code>` は不可。`style` 属性は保持される。

`<code>` の代替は `<strong>` にインラインスタイルを当て、`font-weight:normal` で
太字を打ち消してコードチップを再現する。

```html
<strong style="background:#f4f4f4;color:#c7254e;padding:2px 6px;border-radius:3px;font-family:Consolas,Monaco,monospace;font-size:13px;font-weight:normal;">ECAUTH_DISABLE_ADMIN_PASSWORD_LOGIN</strong>
```

新しいタグを使いたくなったら、まず 1 つだけ入れて保存し、弾かれないか確かめること。
エラーは**弾かれたタグ名を列挙してくれる**ので切り分けは容易。

### CSS クラスに頼らずインラインスタイルで書く

ストア側の CSS クラス名は不明で、変わる可能性もある。すべてインラインスタイルで指定する。
`style` が将来除去されても `<strong>` の強調と `<ul>` の箇条書きは残るので、読める状態に
劣化するだけで済む。

**`<ul>` には `list-style:disc` を明示する。** EC-CUBE 系の CSS は `li { list-style:none }`
を当てていることが多く、指定しないと箇条書きの点が消える。

見出しは `<h4>` / `<h5>` を使わない（ストア側 CSS の影響を受ける）。`<div>` にインライン
スタイルで代用する。EC-CUBE のブランド色は `#fc0`（黄）と `#353a4e`（ダークネイビー）。

```html
<div style="margin:26px 0 10px;padding-left:12px;border-left:4px solid #fc0;font-size:16px;font-weight:bold;color:#353a4e;">設定方法</div>
```

### 手順のコード例は「そのままコピペできる形」にする

`echo 'FOO=1' >> .env` のようなシェルコマンドを載せると、**利用者がその行ごと `.env` に
貼り付ける**。追記させたい 1 行だけを示すこと。

```
NG: echo 'ECAUTH_DISABLE_ADMIN_PASSWORD_LOGIN=1' >> .env
OK: ECAUTH_DISABLE_ADMIN_PASSWORD_LOGIN=1
```

同じ理由で、取りうる値を網羅的に列挙しない（`1` / `true` / `on` / `yes` のいずれか…）。
「有効にするには 1 行追記」「戻すには `0` にするか行を削除」の 2 択に絞る方が事故が少ない。

### HTML 特殊文字は実体参照にする

`>` を含む文（「管理画面 > 設定 > EcAuth」など）は `&gt;`、シングルクォートは `&#39;` に置き換える。

### 文章を書くときの観点

- **既定の挙動を先に書く**。「既定では従来どおりパスワードでログインできます」の一文が
  ないと、既存利用者がアップグレードを躊躇する
- **ロックアウトしうる設定は警告ボックスで囲む**（背景 `#fff8e6` / 枠 `#f0c36d` 等）
- 商品ページはセクションごとに「タイトル」「自由記入」「画像」の 3 枠がある。
  自由記入の冒頭にセクションタイトルと同じ見出しを書くと**重複するので書かない**
