...
test.describe.serial('E2E: パスキー登録からログイン完了までのフロー', () => {
  ...
  test('パスキーを新規登録する', async () => {
    ...
    // 登録完了後は window.location.reload() 後に #ecauth-passkey-add を含む card の table に
    // 登録済みパスキーの行が表示される。EC-CUBE 本体の Web Debug Toolbar にも table が
    // 含まれるため、プラグインの card 内 .table-sm に限定してマッチさせる。
    const passkeyRows = page.locator('.card-body table.table-sm tbody tr');
    await expect(passkeyRows).toHaveCount(1, { timeout: 30000 });
  });
